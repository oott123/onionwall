...
<?php
	...
	for ($i=count($data['statuses'])-1; $i >= 0; $i--) { 
		...
		if(isset($datum['retweeted_status'])){
			continue;
		}
		$weibo_at_id = $datum['idstr'];
	...
	foreach($data['statuses'] as $datum){
		...
		if(!$weibo_at_id) $weibo_at_id = $datum['idstr'];
		$datum['name'] = $datum['user']['screen_name'];
		$datum['avatar'] = $datum['user']['avatar_large'];
		$datum['message'] = $datum['text'];
		save_item($datum);
	...
	*/
	if($weibo_at_id) save_meta('weibo_at_id',$weibo_at_id);
